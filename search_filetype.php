...
<head>
<meta charset="utf-8">
<title>D.M.S - Search (File Type)</title>
...
<?php
	...
	echo '<h1 class="page-head-line">Database Management System - Search (File Type)</h1>';
	echo '<p class="page-head-line"><em>Follow the instructions below to search for the files in the databse via their file type.</em></p>';
	echo '<hr>';
	...
	echo '<input type="text" class="form-control" name="fileType">';
	...
	if (isset($_POST['submit']) && $_POST['submit'] == 'submit') {
		...
		$dblink = db_connect('docstorage');
		$match_sql = "SELECT `file_type` FROM `file_types` WHERE `file_type` = '$fileType'";
		$match_result = $dblink->query($match_sql);
		if ($match_result->num_rows <= 0) {
			redirect('https://ec2-3-15-27-253.us-east-2.compute.amazonaws.com/search_filetype.php?msg=noResult');
		}
		else { // showcase table of the files located
			// get the files
			$fetch_sql = "SELECT * FROM `files` WHERE `file_type` LIKE '$fileType%'";
			$fetch_result = $dblink->query($fetch_sql);
			echo '<h2>Matches for '.$fileType.': '.$fetch_result->num_rows.'</h2>';
